...
# Deploy Path
DEPLOY_DIR = BOARD + 'bsp/rootfs_part/home/ubuntu/pynq/'
DEPLOY_LIB = BOARD + 'bsp/rootfs_part/home/ubuntu/pynq/lib/'
...
def run_make(src_path, dst_path, output_lib):
	...
	if status is not 0:
        ...
        sys.exit(1)
    else: shutil.copyfile(src_path + output_lib, dst_path + output_lib)

# Copy pynq files from src to dest
def cp_pynq(src_path,dst_path):
	for i in range(0,len(src_path)):
		shutil.copyfile(src_path[i], dst_path + '.')
...
def main():
	...
	# Build Pynq Libraries, copy to DEPLOY_LIB
	if CPU_ARCH_IS_SUPPORTED:
	   run_make("pynq/lib/_pynq/_apf/", DEPLOY_LIB, "libdma.so")
	   run_make("pynq/lib/_pynq/_audio/", DEPLOY_LIB, "libaudio.so")
	   run_make("pynq/libsds/xlnkutils/", DEPLOY_LIB, "libsds_lib.so")
	...
	# Copy Supported pynq libs to DEPLOY_LIB
	cp_pynq([pynq_modules,DEPLOY_LIB])
